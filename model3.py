...
from functions import foldRatio, fingVec, extrFing
...
model_1 = NN(50, 24)
model_1.load_state_dict(torch.load("./models/nnEngFeat"))
model_1.eval()
...
while True:
    ...
    if res.multi_hand_landmarks:
        ...
        if imgCrop.size != 0:
            ...
            if result.multi_hand_landmarks:
                fingers = extrFing(result.multi_hand_landmarks)
                vec = fingVec(fingers)
                rat = foldRatio(fingers)
                vec = vec.reshape(-1)
                feat = np.concatenate((vec, rat))
                if feat.shape[0] == 50:
                    inp = torch.from_numpy(feat).float()

                    with torch.no_grad():
                        out = np.argmax(model_1(inp).detach().numpy())
    ...
    cv2.putText(imgWhite, label, (50,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
